...
export const FeeTableComponent = ({
  isLoading,
  data,
  pageCount,
  pageSize,
  ...
  const columns = React.useMemo<ColumnDef<FeeEntity>[]>(
    () => [
      {
        accessorKey: 'fee_id',
        header: 'ID',
      ...
      // },
      {
        ...
        cell: ({ getValue }) => {
          return (
            <Group spacing="xs" noWrap>
              <Link href={`/Fees/detail/${getValue()}`}>
                <Button variant="outline" size="sm" color="blue">
                  Show
                </Button>
              </Link>
              <Link href={`/Fees/edit/${getValue()}`}>
  ...
  const reactTable = useReactTable<FeeEntity>({
    data: data,
