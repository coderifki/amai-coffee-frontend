...
export const SubjectTableComponent = ({
  isLoading,
  data,
  pageCount,
  pageSize,
  ...
  const columns = React.useMemo<ColumnDef<SubjectEntity>[]>(
    () => [
      {
      ...
      // },
      {
        ...
        cell: ({ getValue }) => {
          return (
            <Group spacing="xs" noWrap>
              <Link href={`/Subjects/detail/${getValue()}`}>
                <Button variant="outline" size="sm" color="blue">
                  Show
                </Button>
              </Link>
              <Link href={`/Subjects/edit/${getValue()}`}>
  ...
  const reactTable = useReactTable<SubjectEntity>({
    data: data,
