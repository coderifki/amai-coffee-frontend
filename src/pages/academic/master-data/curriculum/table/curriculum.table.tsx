...
export const CurriculumTableComponent = ({
  isLoading,
  data,
  pageCount,
  pageSize,
  ...
  const columns = React.useMemo<ColumnDef<CurriculumEntity>[]>(
    () => [
      {
      ...
      // },
      {
        ...
        cell: ({ getValue }) => {
          return (
            <Group spacing="xs" noWrap>
              <Link href={`/Periods/detail/${getValue()}`}>
                <Button variant="outline" size="sm" color="blue">
                  Show
                </Button>
              </Link>
              <Link
                href={`/academic/master-data/curriculum/${getValue()}/edit`}
              >
  ...
  const reactTable = useReactTable<CurriculumEntity>({
    data: data,
