...
export const FacultyTableComponent = ({
  isLoading,
  data,
  pageCount,
  pageSize,
  ...
  const columns = React.useMemo<ColumnDef<FacultyEntity>[]>(
    () => [
      {
      ...
      // },
      {
        ...
        cell: ({ getValue }) => {
          return (
            <Group spacing="xs" noWrap>
              <Link href={`/Facultys/detail/${getValue()}`}>
                <Button variant="outline" size="sm" color="blue">
                  Show
                </Button>
              </Link>
              <Link href={`/Facultys/edit/${getValue()}`}>
  ...
  const reactTable = useReactTable<FacultyEntity>({
    data: data,
