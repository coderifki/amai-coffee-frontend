...
export const TeacherTableComponent = ({
  ...
  const columns = React.useMemo<ColumnDef<TeacherEntity>[]>(
    () => [
      {
      ...
      // },
      {
        ...
        cell: ({ getValue }) => {
          return (
            <Group spacing="xs" noWrap>
              <Link href={`/Teachers/detail/${getValue()}`}>
                <Button variant="outline" size="sm" color="blue">
                  Show
                </Button>
              </Link>
              <Link href={`/Teachers/edit/${getValue()}`}>
  ...
  const reactTable = useReactTable<TeacherEntity>({
    data: data,
